                        ...
                        case 1:
                            {
                                ...
                                else
                                {
                                    ...
                                    #region 判断是否需要新增员工
                                    var strCompany = record.Value<string>("vEmpCompany");
                                    var strDepartment = record.Value<string>("vEmpDepartment");
                                    var strEmpPhone = record.Value<string>("vEmpPhone");
                                    var empName = record.Value<string>("vEmpName");
                                    if (!string.IsNullOrWhiteSpace(strCompany)
                                        && !string.IsNullOrWhiteSpace(strDepartment)
                                        && !string.IsNullOrWhiteSpace(strEmpPhone)
                                        && !string.IsNullOrWhiteSpace(empName))
                                    {
                                        ...
                                        if (!bll_employ.ExistEmpPhone_API(strEmpPhone))
                                        {
                                            //新增员工
                                            M_Employ_Info newEmploy = new M_Employ_Info();
                                            newEmploy.CompanyId = curCompany.CompanyId;
                                            newEmploy.DeptNo = curDeptment.DeptNo;
                                            newEmploy.EmpName = record.Value<string>("vEmpName");  //vEmpName
                                            newEmploy.EmpMobile = record.Value<string>("vEmpPhone");  //vEmpPhone;
                                            var newEmpNo=bll_employ.Add_wx(newEmploy);
                                            if (newEmpNo > 0)
                                                booking.EmpNo = newEmpNo;
