...
namespace ADServer.BLL
{
    public static class INetFwManger
    {
        ...
        public static void AddRule(string ruleName, int port)
        {
            string str = " netsh advfirewall firewall add rule name=" + ruleName + " dir=in action=allow protocol=TCP localport= " + port.ToString();
        ...
        {
            string strDelete = " netsh advfirewall firewall delete rule name=" + ruleName + " dir=in";
            System.Diagnostics.Process pro = new System.Diagnostics.Process();//实例化进程
            pro.StartInfo.FileName = "cmd.exe";//设置要运行的程序文件
            pro.StartInfo.UseShellExecute = false;//是否使用操作系统shell程序启动
            pro.StartInfo.RedirectStandardInput = true;//是否接受来自应用程序的调用
            ...
            //向cmd窗口发送输入信息
            pro.StandardInput.WriteLine(strDelete + "&exit");
        ...
        public static void NetFwAddPorts(string name, int port, string protocol)
        {
            ...
            foreach (INetFwOpenPort mPort in netFwMgr.LocalPolicy.CurrentProfile.GloballyOpenPorts)
            {
                if (objPort.Name == mPort.Name)
                {
                    ...
                    netFwMgr.LocalPolicy.CurrentProfile.GloballyOpenPorts.Remove(mPort.Port, NET_FW_IP_PROTOCOL_.NET_FW_IP_PROTOCOL_TCP);
